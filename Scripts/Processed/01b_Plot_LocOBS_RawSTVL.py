...
#############################################################
# CODE DESCRIPTION
# 01b_Plot_LocOBS_RawSTVL.py plots a map with an example of the
...
DirOUT = "Data/Plot/01b_LocOBS_RawSTVL"
#############################################################
...
for Dataset in Dataset_list:
    ...
    TempFile = Git_repo + "/" + DirIN + "/" + Dataset + "/" + Date.strftime("%Y%m%d") + "/tp" + str(Acc) + "_obs_" + Date.strftime("%Y%m%d")
...
title = mv.mtext(
    text_line_count = 2,
    text_line_1     = "Example of the location of " + str(Acc) + "-hourly rainfall observations for " + Date.strftime("%Y-%m-%d"),
    text_line_2     = " ",
...
png = mv.png_output(output_name = MainDirOUT + "/LocOBS_" + Date.strftime("%Y%m%d"))
mv.setoutput(png)
