...
#######################################################################################################################################
# CODE DESCRIPTION
# 07_Compute_AlignedOBS-Extract-GridCPC.py extracts the gridded rainfall values from the "CPC Global Unified Gauge-Based Analysis of Daily Precipitation" dataset 
...
# DirOUT (string): relative path for the output directory that will contain the gridded CPC rainfall values for the nearest grid point to the aligned STVL rain stations
...
DirOUT = "Data/Compute/07_AlignedOBS-Extract-GridCPC"
#######################################################################################################################################

np.set_printoptions(suppress=True, formatter={'float_kind':'{:0.2f}'.format})

# Setting main input/output directories
MainDirIN_CPC = "/home/mofp"
# MainDirIN_CPC = Git_repo + "/" + DirIN_CPC + "_" + str(Acc) + "h"
MainDirIN_STVL = Git_repo + "/" + DirIN_STVL
...
FileIN_CPC_temp = MainDirIN_CPC + "/precip." + str(YearS) + ".nc"
FileIN_CPC_temp = "/home/mofp/precip.2000.nc"
cpc_metadata = nc.Dataset(FileIN_CPC_temp, "r")
print(cpc_metadata)
...
print("The online plot taken as an example was saved in " + MainDirOUT + " but it can also be recreated online here: https://psl.noaa.gov/mddb2/makePlot.html?variableID=2781 ")
...
print(MainDirOUT)
np.save(MainDirOUT + "/stn_ids.npy", stvl_stnids)
np.save(MainDirOUT + "/stn_lats.npy", stvl_lats)
np.save(MainDirOUT + "/stn_lons.npy", stvl_lons)
np.save(MainDirOUT + "/dates.npy", stvl_dates)
np.save(MainDirOUT + "/obs.npy", cpc_obs)